...
const MessageCard = ({
    content, role, type, createdAt, fragment, isActiveFragment, onFragmentClick}:MessageProps
) => {
...
const AssistantMessage = ({
    content, fragment, type, createdAt, isActiveFragment, onFragmentClick}: AssistantMessageProps
) =>{
    return(
...
const FragmentCard = ({fragment, isActiveFragment, onFragmentClick}:FragmentCardProps) => {
    return(
        <button 
        className={cn("flex items-start cursor-pointer  text-start gap-2 border rounded-lg bg-muted w-fit p-3 hover:bg-secondary transition-colors", 
